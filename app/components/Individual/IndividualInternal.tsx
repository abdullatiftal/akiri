...
import {
    IndividualInternalProps,
    IndividualInternalFormValues
} from '../../types'
...
export const IndividualInternal: FC<IndividualInternalProps> = ({ title }) => {
    const { onHandleNext, onHandlePrev, formData, setFormData } = useFormState()
    const { control, register, handleSubmit } =
        useForm<IndividualInternalFormValues>({ defaultValues: formData })
    const onHandleFormSubmit = (data: IndividualInternalFormValues) => {
